...
- **Positive (+):** async end-to-end (routes, DB session, JWT checks); free interactive docs at
  `/docs`; Pydantic v2 reused for both config (`pydantic-settings`) and schemas, one validation
  library for the whole app.
- **Negative (-):** FastAPI's dependency-injection style (`Depends()`) is framework-specific and
...
- **Positive (+):** no raw SQL anywhere in the codebase; schema changes are versioned,
...
- **Negative (-):** async SQLAlchemy 2.0 has a smaller ecosystem/less mature docs than the sync
...
- **Positive (+):** tests run with no Postgres container, fully isolated per test, fast enough
...
- **Positive (+):** one lint/format tool (ruff) instead of three; hooks run in the real project
...
- **Negative (-):** no CI pipeline yet, so enforcement depends on every contributor having hooks
...
- **Positive (+):** small runtime image (no build toolchain shipped); non-root, read-only,
...
- **Negative (-):** no TLS termination in-app (assumes a reverse proxy/ingress in front, per
...
- **Positive (+):** access-token revocation is immediate (checked on every request via
...
- **Negative (-):** adds Redis as a new runtime dependency (`docker-compose.yml`, `REDIS_*` env
...
- **Positive (+):** list CRUD (create/get/update/delete) is fully implemented and tested,
...
- **Negative (-):** the initial version of this change shipped without the `list_id` FK on
...
- **Positive (+):** every task now has exactly one owning list, enforced at the DB level (FK,
...
- **Negative (-):** a task can never be created without a list (no "inbox"/unlisted tasks), and
...
- **Positive (+):** filtered listing and completion percentage are both testable in isolation
...
- **Negative (-):** `completion_percentage` costs two extra `COUNT` queries per request (on top
...
- **Positive (+):** a responsible user per task is verifiable end-to-end (unit tests in
...
- **Negative (-):** there's no restriction that the assigned user belong to the same
...
- **Positive (+):** the fake send is tested end-to-end (unit test in `NotificationService` +
...
- **Negative (-):** there's no endpoint to list or query sent invitations (out of scope for
...
- **Positive (+):** the service-level check produces a client-readable 409 with
...
- **Negative (-):** the duplicate check is "read then write", not atomic; under high concurrency
...
- **Positive (+):** local onboarding goes from "copy file + 5 commands + manual editing" to a
...
- **Negative (-):** this is a local-development-only convenience — secrets generated by
